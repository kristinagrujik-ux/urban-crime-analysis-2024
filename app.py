...
# 3.35 СПЕЦИЈАЛИЗИРАН ПРИКАЗ ЗА ТРГОВИЈА СО ЛУЃЕ
    ...
    # Блок 1: Трговија со луѓе (земаме редици од 2 до 5, со колони за години 3, 6 и 8 според вашата табела)
    try:
    ...
    # Блок 2: Трговија со деца (земаме редици од 13 до 16, со истите индекси на колони)
    try:
    ...
    if not blocks:
        st.error("Не можат да се пронајдат податоците за трговија со луѓе и деца.")
        st.dataframe(df, use_container_width=True)
    else:
        ...
        color_palettes = [
            ["#1f77b4", "#6baed6", "#c6dbef"],  # Прв график: Трговија со луѓе (сини нијанси)
            ["#d62728", "#f4a582", "#fddbc7"],  # Втор график: Трговија со деца (црвени/портокалови нијанси)
        ]
