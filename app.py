...
# 4.7 СПЕЦИЈАЛИЗИРАН ПРИКАЗ ЗА ТРГОВИЈА СО ЛУЃЕ (И ДЕЦА) - Поправено да фаќа и со латиница и со ћирилица / мали букви
    ...
    with col1:
        ...
        melted1 = df_table1.melt(id_vars=['Кривични дела'], value_vars=['2024 година', '2023 година', '2022 година'], var_name='Година', value_name='Број')
        base1 = alt.Chart(melted1).encode(
            x=alt.X('Кривични дела:N', title=None, sort=['Број на акциски контроли', 'Број на угостителски објекти', 'Број на странски државјани'], axis=alt.Axis(labelAngle=0, labelLimit=200)),
            y=alt.Y('Број:Q', title='Број'),
        ...
        st.altair_chart((bars1 + text1).properties(height=380), use_container_width=True)

    with col2:
        st.write("**Трговија со деца: 2024 vs 2023 vs 2022 година**")
        melted2 = df_table2.melt(id_vars=['Кривични дела'], value_vars=['2024 година', '2023 година', '2022 година'], var_name='Година', value_name='Број')
        base2 = alt.Chart(melted2).encode(
            x=alt.X('Кривични дела:N', title=None, sort=['Број на кривични дела', 'Број на сторители', 'Број на жртви'], axis=alt.Axis(labelAngle=0, labelLimit=200)),
            y=alt.Y('Број:Q', title='Број'),
        ...
        st.altair_chart((bars2 + text2).properties(height=380), use_container_width=True)
