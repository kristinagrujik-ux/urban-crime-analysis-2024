...
# 2. ОСТАНАТИ КАТЕГОРИИ
else:
    st.subheader("📈 Графикони")

    # Специфични графикони за Вкупен криминалитет
    if "Вкупен" in selected_sheet:
        ...
        ef_col = valid_rows.columns[6]    # Расветлени КД

        col1, col2 = st.columns(2)
        with col1:
            st.write("**Вкупен криминалитет за 2024 (Column Chart)**")
        ...
        with col2:
            st.write("**Сторители (Horizontal Bar)**")
            st.altair_chart(alt.Chart(valid_rows).mark_bar(color='#ff7f0e').encode(
                y=alt.Y(f'{sector_col}:N', title=None),
                x=alt.X(f'{s_col}:Q', title='Број')
            ).properties(height=300), use_container_width=True)

        col3, col4 = st.columns(2)
        with col3:
            st.write("**Стапка на криминалитет (Line Chart)**")
            st.altair_chart(alt.Chart(valid_rows).mark_line(point=True, color='#2ca02c').encode(
                x=alt.X(f'{sector_col}:N', title=None, axis=alt.Axis(labelAngle=270)),
                y=alt.Y(f'{st_col}:Q', title='Стапка', scale=alt.Scale(zero=False))
            ).properties(height=300), use_container_width=True)

        with col4:
            st.write("**Вкупна ефикасност 2024 (Horizontal Bar)**")
            st.altair_chart(alt.Chart(valid_rows).mark_bar(color='#d62728').encode(
                y=alt.Y(f'{sector_col}:N', title=None),
                x=alt.X(f'{ef_col}:Q', title='Расветлени КД')
            ).properties(height=300), use_container_width=True)

    # Стандарден приказ за други листови
    else:
