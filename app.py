...
if "Криумчарење" in selected_sheet or "мигранти" in selected_sheet.lower():
    ...
    with col2:
        ...
        bar_chart = alt.Chart(df_mig_pct).mark_bar(color='#d62728').encode(
            y=alt.Y('Категорија:N', title='Категорија', sort=alt.EncodingSortField(field='Процент', op='min', order='ascending'), axis=alt.Axis(labelAngle=0, labelLimit=300)),
            x=alt.X('Процент:Q', title='Процент (%)')
        )
        text_chart = alt.Chart(df_mig_pct).mark_text(align='left', dx=5).encode(
            y=alt.Y('Категорија:N', sort=alt.EncodingSortField(field='Процент', op='min', order='ascending')),
            x=alt.X('Процент:Q'),
            text='Пр_Текст:N'
        )
