...
# 4.7 СПЕЦИЈАЛИЗИРАН ПРИКАЗ ЗА ТРГОВИЈА СО ЛУЃЕ
    ...
    if header_row_idx is None:
        ...
        # Разделуваме на "Трговија со луѓе" (општо) и "Трговија со деца"
        deca_mask = trg_clean['Категорија'].astype(str).str.contains('дец', case=False, na=False)
        trg_lugje = trg_clean[~deca_mask].reset_index(drop=True)
        trg_deca = trg_clean[deca_mask].reset_index(drop=True)

        col1, col2 = st.columns(2)
        with col1:
            st.write("**Трговија со луѓе: 2024 vs 2023 vs 2022 година**")
            if not trg_lugje.empty:
                order1 = trg_lugje['Категорија'].tolist()
                melted1 = trg_lugje.melt(id_vars=['Категорија'], value_vars=['2024 година', '2023 година', '2022 година'], var_name='Година', value_name='Број')
                chart1 = alt.Chart(melted1).mark_bar().encode(
                    x=alt.X('Категорија:N', title=None, sort=order1, axis=alt.Axis(labelAngle=270, labelLimit=200)),
                    y=alt.Y('Број:Q', title='Број'),
                    color=alt.Color('Година:N', scale=alt.Scale(domain=['2024 година', '2023 година', '2022 година'], range=['#1f77b4', '#6baed6', '#c6dbef']), legend=alt.Legend(title="Година")),
                    xOffset='Година:N'
                )
                st.altair_chart(chart1.properties(height=380), use_container_width=True)
            else:
                st.info("Нема податоци за Трговија со луѓе.")

        with col2:
            st.write("**Трговија со деца: 2024 vs 2023 vs 2022 година**")
            if not trg_deca.empty:
                order2 = trg_deca['Категорија'].tolist()
                melted2 = trg_deca.melt(id_vars=['Категорија'], value_vars=['2024 година', '2023 година', '2022 година'], var_name='Година', value_name='Број')
                chart2 = alt.Chart(melted2).mark_bar().encode(
                    x=alt.X('Категорија:N', title=None, sort=order2, axis=alt.Axis(labelAngle=270, labelLimit=200)),
                    y=alt.Y('Број:Q', title='Број'),
                    color=alt.Color('Година:N', scale=alt.Scale(domain=['2024 година', '2023 година', '2022 година'], range=['#d62728', '#f4a582', '#fddbc7']), legend=alt.Legend(title="Година")),
                    xOffset='Година:N'
                )
                st.altair_chart(chart2.properties(height=380), use_container_width=True)
            else:
                st.info("Нема податоци за Трговија со деца.")
