...
# 4.8 СПЕЦИЈАЛИЗИРАН ПРИКАЗ ЗА ТЕШКИ КРАЖБИ
    ...
    if header_row_idx is None:
        ...
        with col2:
            ...
            base_kd_bar = alt.Chart(krazbi_clean).encode(
                y=alt.Y('СВР:N', title=None, sort=sector_order),
                x=alt.X('Промена:Q', axis=alt.Axis(format='%'), title='Промена')
            )
            bars_kd_bar = base_kd_bar.mark_bar(size=14, color='#5B84B1')
            text_pos_bar = base_kd_bar.transform_filter(alt.datum.Промена >= 0).mark_text(align='left', dx=5, fontSize=11).encode(text='Промена текст:N')
            text_neg_bar = base_kd_bar.transform_filter(alt.datum.Промена < 0).mark_text(align='right', dx=-5, fontSize=11).encode(text='Промена текст:N')
            st.altair_chart((bars_kd_bar + text_pos_bar + text_neg_bar).properties(height=380), use_container_width=True)
